...
// --- DATA GENERATORS ---
const generateVehicles = (count = 15) => { // User Request: 15 Vehicles fixed
...
const generateCompanies = (count = 80) => { // User Request: 80 Clients fixed
  const NAMES = [
...
function App() {
    ...
    const initSystem = async () => {
      ...
      if (cloudData) {
        ...
        if ((cloudData.simulationMetadata?.config?.DAYS_TO_SIMULATE || 0) < 360) {
          ...
          if (cloudData.vehicles) setVehicles(cloudData.vehicles.slice(0, 15));
  ...
  const handleRegenerateSimulation = async (configOverride) => {
    ...
    if (newData) {
      ...
      if (newData.vehicles) setVehicles(newData.vehicles.slice(0, 15));
  ...
  async function simularDiaCritico() {
    ...
    try {
      ...
      if (newData) {
        ...
        setVehicles(mappedVehicles.slice(0, 15));
