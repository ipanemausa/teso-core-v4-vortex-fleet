      ...
      <section id="map-container" style={{ position: 'relative', background: '#0a0f1a', height: '100%' }}>
        ...
        <ErrorBoundary>
          ...
          >
            ...
            {/* STATIC AIRPORTS */}
            <Marker position={[6.1645, -75.4231]} icon={getAirportIcon('MDE')} eventHandlers={{ click: () => addLog('🏢 JMC: OPERACIONES NORMALES') }} />
            <Marker position={[6.2205, -75.5898]} icon={getAirportIcon('EOH')} eventHandlers={{ click: () => addLog('🏢 EOH: OPERACIONES NORMALES') }} />
            ...
            <ConnectionLines highlighted={highlighted} vehicles={vehicles} requests={requests} />
            ...
            {planes.map(p => (
              <Marker key={p.id} position={[p.lat, p.lng]} icon={p.status === 'LANDED' ? parkedPlaneIcon : planeDivIcon(p.heading)}>
                <Tooltip direction="right" offset={[10, 0]} opacity={0.8}>
        ...
        {/* 2. RIGHT PANEL (TESO OPS CONSOLE) - RESTORED & CLEANED */}
        {!showOperationalDashboard && !showLanding && (
          <div style={{
        ...
        {/* 3. V6 COMMAND DOCK (Floating Bottom Right) */}
        {!showOperationalDashboard && !showLanding && (
          <div style={{
            position: 'absolute', bottom: '30px', right: '370px', // Left of Sidebar
            display: 'flex', alignItems: 'center', gap: '20px', zIndex: 999
          }}>
            {/* COMMANDS */}
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => addLog('OPTIMIZING ROUTES...')} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid #00F0FF', color: '#00F0FF', padding: '8px 15px', borderRadius: '20px', fontSize: '0.8rem', cursor: 'pointer' }}>✨ Optimize</button>
              <button onClick={() => addLog('AUDIT RUNNING...')} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid gold', color: 'gold', padding: '8px 15px', borderRadius: '20px', fontSize: '0.8rem', cursor: 'pointer' }}>✨ Audit</button>
              <button onClick={() => addLog('SECURITY SCAN...')} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid #39FF14', color: '#39FF14', padding: '8px 15px', borderRadius: '20px', fontSize: '0.8rem', cursor: 'pointer' }}>✨ Security</button>
            </div>
          </div>
        )}

        {/* 4. CHAT BAR (Ask Teso Operations AI...) - RESTORED */}
        {!showOperationalDashboard && !showLanding && (
          <div style={{
            position: 'absolute', bottom: '30px', left: '20px', right: 'auto', // Bottom Left
            zIndex: 999, display: 'flex', alignItems: 'center', gap: '10px'
          }}>
            <div style={{
              background: 'rgba(0,0,0,0.9)', border: '1px solid #00F0FF', borderRadius: '50px',
              padding: '12px 25px', display: 'flex', alignItems: 'center', gap: '15px',
              width: '500px', boxShadow: '0 0 20px rgba(0, 240, 255, 0.2)'
            }}>
              <span style={{ fontSize: '1.5rem' }}>🤖</span>
              <input
                placeholder="Ask Teso Operations AI..."
                style={{ background: 'transparent', border: 'none', color: '#fff', width: '100%', outline: 'none', fontSize: '1rem' }}
              />
              <span style={{ cursor: 'pointer' }}>🎤</span>
              <button style={{ background: '#00F0FF', border: 'none', borderRadius: '50%', width: '35px', height: '35px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>➤</button>
            </div>
          </div>
        )}

        {/* 5. RIGHT PANEL (TESO OPS SIDEBAR) - RESTORED INSIDE MAP CONTAINER */}
        {!showOperationalDashboard && !showLanding && (
          <div className="teso-ops-sidebar" style={{
            position: 'absolute', top: '0', right: '0', bottom: '0', width: '350px',
            background: 'rgba(5, 10, 20, 0.95)', borderLeft: '1px solid #333',
            zIndex: 1000, overflowY: 'auto', padding: '20px',
            display: 'flex', flexDirection: 'column', gap: '20px'
