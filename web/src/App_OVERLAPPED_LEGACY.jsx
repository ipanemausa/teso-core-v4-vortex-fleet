...
import { queryErpRag } from './modules/rag_erp/ErpRagCore.js';
import LandingPage from './components/LandingPage';
import Presentation from './components/Presentation';
import TripPreferencesModal from './components/TripPreferencesModal'; // NEW: Growth Spike
import SharedRideView from './components/SharedRideView'; // NEW: Safety Spike
...
import pdfSierService from './assets/documents/invoices/invoice_sier_service.pdf';
import pdfViajesAz from './assets/documents/invoices/invoice_viajes_az.pdf';
import pdfPersonalSoft from './assets/documents/invoices/invoice_personal_soft.pdf';
...
const getAirportIcon = (code) => new L.DivIcon({
...
const parkedPlaneIcon = new L.DivIcon({
...
function App() {
  ...
  const [simulationContext, setSimulationContext] = useState({
    ...
    expenses: [],
    payables: [],
    conflicts: { cancellations: 0, delays: 0, driverChanges: 0 },
    services: [],
    clients: []
  }); // SAFE INIT: Empty Structure to prevent crashes
  ...
  const corporateAccounts = [
    { name: 'SIER SERVICE', pdf: pdfSierService, last: 'FAC-001' },
    { name: 'PERSONAL SOFT', pdf: pdfPersonalSoft, last: 'FAC-003' }
  ];
  ...
  const mapServerDataToFrontend = (serverData) => {
    ...
    // Map Services
    const mappedServices = (serverData.services || []).map(svc => ({
      id: svc.ID,
      date: svc.FECHA,
    ...
    const mappedBankTx = (serverData.cash_flow || []).map(row => ({
      date: row.FECHA,
  ...
  const getApiUrl = () => {
    ...
    // Priority 2: Current Hostname (Local Network/Localhost)
    const hostname = window.location.hostname;
    return `http://${hostname === 'localhost' ? 'localhost' : hostname}:8000`;
  };
    ...
    const initSystem = async () => {
      ...
      if (cloudData) {
        ...
        if ((cloudData.simulationMetadata?.config?.DAYS_TO_SIMULATE || 0) < 360) {
          ...
          setRequests(cloudData.services.slice(-60));
          localStorage.setItem('TESO_SIM_STATE_V2', JSON.stringify(cloudData));
        }
      } else {
        ...
        if (freshData) setSimulationContext(freshData);
      }
    };
  ...
  const handleRegenerateSimulation = async (configOverride) => {
    ...
    if (newData) {
      ...
      setRequests(newData.services.slice(-60)); // Refresh live view
      setIsScanning(false);
  ...
  if (showOperationalDashboard) {
    return (
      <OperationalDashboard
        ...
        simulationData={simulationContext} // PASSING THE CONTEXT
        onClose={() => setShowOperationalDashboard(false)} // NEW: Close Handler
        onRegenerate={handleRegenerateSimulation} // NEW: Dynamic Scaling Handler
        onSimulateStress={simularDiaCritico} // PASSED DOWN
        onRunMacro={runMacroSequence} // PASSED DOWN
        onRowClick={(req) => {
          setShowOperationalDashboard(false);
          const v = vehicles.find(v => v.id === req.assignedVehicle);
          if (v) {
            setActiveReq({ ...v, ...req }); // Merge for Tactical Card
          }
          setActiveRequestId(req.id); // Focus map
        }}
      />
    );
  }
  ...
  const handleAgentCommand = (cmd) => {
    ...
    setTimeout(() => {
      ...
      if (lower.includes('financ') || lower.includes('audit')) {
        ...
        speak("Modo Auditoría Financiera activado. Verificando márgenes y flujo de caja.");
        ...
        speak("Accediendo al Núcleo Operativo. Visualizando flota en tiempo real.");
      } else if (lower.includes('excel') || lower.includes('planilla') || lower.includes('programacion')) {
        addLog(`📊 AGENT: Generando Planilla Master (Multi-hoja) para Administración...`);
        setDashboardViewMode('ANALYTICS');
        setShowOperationalDashboard(true);
        addLog(`✅ AGENT: Desplegando Planilla Master Virtual en Tiempo Real.`);
        speak("Generando Planilla Máster. Datos sincronizados con hoja de cálculo.");
      } else if (lower.includes('secur') || lower.includes('scan')) {
        setDashboardViewMode('LIVE_OPS');
        setShowOperationalDashboard(true);
        setIsScanning(true);
        addLog(`✅ AGENT: Security scan initiated. Verifying 65 units...`);
        speak("Iniciando escaneo de seguridad. Verificando antecedentes y documentos de 65 unidades.");
        setTimeout(() => setIsScanning(false), 5000);
      } else if (lower.includes('demo') || lower.includes('iniciar')) {
        runAppDemoMode(); // MANUAL PRESENTATION -> AUTO APP DEMO
      } else if (lower.includes('auto') || lower.includes('complete')) {
        runMacroSequence(); // FULL HANDS-FREE MODE
        setShowWebcam(prev => !prev);
        addLog(`🎥 AGENT: Toggling Presenter Camera.`);
      } else if (lower.includes('hola') || lower.includes('aló') || lower.includes('hello')) {
        addLog(`👋 AGENT: ¡Hola! Sistemas de Teso en línea y listos para la demostración.`);
        speak("Hola. Soy el Agente Teso. Estoy listo para nuestra demostración.");
      } else if (lower.includes('rodeo') || lower.includes('stress') || lower.includes('simulacion') || lower.includes('big bang')) {
        addLog(`💥 AGENT: INICIANDO PROTOCOLO "RODEO" (STRESS TEST)...`);
        speak("Iniciando Protocolo Rodeo. Simulando 30 días de operación masiva.");

        // EXECUTE SIMULATION
        const simData = runSimulation();

        // MAPPING DRIVERS TO VEHICLE FORMAT EXPECTED BY MAP
        // Simulation drivers have { id, name, plate, status, location }
        // App vehicle state expects { id, lat, lng, status, driver, ... }
        ...
        const mappedClients = simData.clients.map(c => ({
          id: c.id,
          name: c.companyName,
          type: c.tier === 'VIP' ? 'PLATINUM' : 'CORPORATIVO',
          tier: c.tier,
          budget: '$Simulated'
        }));

        // INJECT DATA
        // STORE FULL CONTEXT FOR DASHBOARD
        setSimulationContext({
          bankTransactions: simData.bankTransactions,
          expenses: simData.expenses,
          payables: simData.payables,
          conflicts: simData.conflicts,
          services: simData.services, // Needed for Big Data Analytics
          clients: simData.clients    // Needed for Big Data Client Resolution
        });

        // MAPPING SERVICES TO REQUESTS FORMAT
        // App expects { id, fare, status, type, financials... }
        const mappedRequests = simData.services.map(s => {
          // ORGANIC DISTRIBUTION (POLAR)
          // Center roughly on Medellín (6.24, -75.58)
          const angle = Math.random() * 2 * Math.PI;
          const radius = Math.sqrt(Math.random()) * 0.08; // 0.08 degrees radius (~9km)
          const lat = 6.24 + (radius * Math.cos(angle));
          const lng = -75.58 + (radius * Math.sin(angle));
          ...
          return {
            ...
            dest: 'Simulated Destination',
            paxName: 'Simulated Pax',
            company: simData.clients.find(c => c.id === s.clientId)?.companyName || 'Unknown',
            client: simData.clients.find(c => c.id === s.clientId)?.companyName || 'Unknown', // Key for Dashboard Aggregation
            status: s.status.toLowerCase(),
            type: 'CREDITO', // Corporate is Credit
            fare: `$${s.financials.totalValue.toLocaleString('es-CO')}`, // Display String
            financials: s.financials, // CRITICAL: The Object we added support for
            assignedDriver: s.driverId ? simData.drivers.find(d => d.id === s.driverId)?.name : 'Unassigned',
            assignedVehicle: 'Simulated Car',
            pin: 'SIM-000'
          };
        });

        // VISUAL FILTER: Only show the last 150 requests on the Map/List
        // This prevents the "Yellow Blob" effect while keeping the "Big Data" in the background
        const displayRequests = mappedRequests.slice(-150);

        setRequests(displayRequests);

        // AUTO-EXPORT BIG DATA (30 DAYS FULL HISTORY)
        try {
          generateMasterExcel(mappedRequests, mappedVehicles, mappedClients, {
            bankTransactions: simData.bankTransactions,
            expenses: simData.expenses,
            payables: simData.payables,
            conflicts: simData.conflicts,
            services: simData.services,
            clients: simData.clients
          });
          addLog(`💾 AGENT: AUTO-SAVED "BIG DATA" EXPORT TO LOCAL DISK.`);
        } catch (err) {
          console.error("Export Error:", err);
          addLog(`⚠️ AGENT: Export failed slightly, but Analytics are safe.`);
        }
        ...
        setTimeout(() => {
          setDashboardViewMode('ANALYTICS');
          setShowOperationalDashboard(true);
          addLog(`✅ SIMULATION COMPLETE: ${simData.services.length} Operations Processed.`);
          speak("Simulación finalizada. Datos financieros cargados en el tablero.");
        }, 3000); // 3 Seconds for better UX (User Request)

      } else if (lower.includes('teso')) {
        // --- NEW: ERP PANAX-STYLE RAG INTEGRATION ---
        // Catch-all for "teso..." questions or just analyze any question mark?
        // Let's make it intuitive: If it didn't match a command, try the RAG.
      }

      // ... FALLBACK TO RAG IF NO COMMAND MATCHED ...
      else {
        // Try to query the ERP Brain
        ...
        if (ragResponse.type !== 'UNKNOWN') {
          ...
          // If simulation data was returned (e.g. forecast chart), switch to Analytics view to show it theoretically
          if (ragResponse.type === 'SIMULATION' || ragResponse.type === 'FINANCIAL_REPORT') {
            // Option: Auto-open dashboard if relevant
            // setShowOperationalDashboard(true);
            // setDashboardViewMode('ANALYTICS');
          }
        } else {
        ...
        showOperationalDashboard && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 5000 }}>
            <OperationalDashboard
              ...
              onRowClick={handleDashboardDrillDown}
              onRegenerate={handleRegenerateSimulation}
